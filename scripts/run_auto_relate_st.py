...
def main():
    ...
    dataset_path = os.environ.get(
        ...
        os.path.join(PROJECT_ROOT, "data", "ST"),
    )
    data_type = os.environ.get("AUTO_RELATE_DATA_TYPE", "all")
    result_save_folder_path = os.environ.get(
        "AUTO_RELATE_RESULT_DIR",
        os.path.join(PROJECT_ROOT, "results", "st"),
    )
    measure_and_save_result.run(dataset_path, data_type, "ST", methods, result_save_folder_path)
